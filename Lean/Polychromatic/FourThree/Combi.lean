...
-/
def blockColorVal (A B : List (Fin 3)) (h _k : ℕ) (p : ℕ) : Fin 3 :=
...
private lemma bcv_eq_A (A B : List (Fin 3)) (h k : ℕ)
    ...
    (_hjs : js < XY.length) (hp : p < A.length * h)
...
private lemma case_no_wrap_AA (A B : List (Fin 3)) (offsets : List ℕ)
  ...
  · exact bcv_eq_A A B h k _ _ _ _ (by grind) his_A
...
private lemma case_no_wrap_AB (A B : List (Fin 3)) (offsets : List ℕ)
  ...
  · exact bcv_eq_A A B h k _ _ _ _
      (by grind) (add_lt_of_mod_add_lt hA hA_region hjs_lt)
  ...
  · push_neg at hjs_lt
    ...
    refine bcv_eq_B A B h k _ _ _ _ (by grind) hnot_A ?_ hjs_B ?_
...
private lemma case_wrap_A (A B : List (Fin 3)) (offsets : List ℕ)
  ...
  · have his_lt : i + s < A.length * h :=
    ...
    exact bcv_eq_A A B h 0 _ _ _ _ (by grind) his_lt
      (add_mod_of_lt hA hjs_lt) hjs_lt (List.getElem?_append_left hjs_lt)
  · push_neg at hjs_lt
    have his_ge : A.length * h ≤ i + s :=
      ge_mul_of_mod_add_ge hA hh_pos hi_lo hjs_lt
    ...
    refine bcv_eq_A A B h 0 _ _ _ _ (by grind) (by grind)
...
private lemma case_wrap_BA (A B : List (Fin 3)) (offsets : List ℕ)
  ...
  · -- Wrapped to A region
    ...
    refine bcv_eq_A A B h k _ _ _ _ (by grind) (by grind)
      (by rw [hmod, Nat.mod_eq_of_lt hsub_lt, hsub_eq])
      hjs_idx ?_
    rw [List.getElem?_append_right
      (by grind : B.length ≤ j + s)]
...
private lemma case_wrap_BB (A B : List (Fin 3)) (offsets : List ℕ)
  ...
  · have his_lt : i + s < B.length * k :=
    ...
    rw [Nat.mod_eq_of_lt (by grind : i + s < m)]
    exact bcv_eq_B A B 0 k _ _ _ _ (by grind)
      (by omega)
  ...
  · push_neg at hjs_lt
    ...
    refine bcv_eq_B A B 0 k _ _ _ _ (by grind)
      (by omega)
...
private lemma straddle2_gap1 (s g m : ℕ)
    ...
    (_hv_lo : equiEndpoint m s j₀ ≤ v)
    (hv_hi : v < equiEndpoint m s (j₀ + 1))
    (_hvg_lo : equiEndpoint m s jg ≤ (v + g) % m)
    (hvg_hi : (v + g) % m < equiEndpoint m s (jg + 1))
...
private lemma eqp_idx_succ_lt_m (q r s p : ℕ)
    ...
    (hm_eq : m = s * q + r)
    (hp : p < m) (_hidx_lt : eqp_idx q r p < s) :
...
private lemma straddle_boundary_color (q r s p : ℕ)
    ...
    (j : ℕ) (hj : j = eqp_idx q r p) (_hj_lt : j < s) :
    (eqp_idx q r ((p + 1) % m) +
  ...
  · -- Pair 1 covers k: k ∈ {j₀%3, (j₀+1)%3}
    ...
    · -- Pair 1 straddles → gap = 2
      ...
      have hgap2 := straddle1_gap2 s g m hs hs3_le hs_le h_lb
        h_ub v j₀ jg hv_lt hj₀_lt hjg_lt hv_lo hv_hi hvg_lo
        hvg_hi hstrad1 hgap
      ...
      · -- k = j₀%3 = (jg+1)%3: pair 2 must be non-straddle
        ...
        · -- Pair 2 non-straddle → witness a = g+d
          ...
          · change (idx ((v + (g + d)) % m) +
            ...
            rw [vg_mod_shift m v g d (by omega)]; exact hd_eq
        · -- Pair 2 straddles → contradiction: gap = 1
          ...
          have hgap1 := straddle2_gap1 s g m hs hs3_le hs_le
            ...
            hvg_lo hvg_hi hstrad2 hgap
          omega
      · -- k = (j₀+1)%3: witness a = 1
        refine ⟨1, by simp, ?_⟩
        change (eqp_idx q r ((v + 1) % m) +
            eqp_off q r ((v + 1) % m) % 2) % 3 = k.val
        rw [straddle_boundary_color m q r s v hq_pos hr_lt hs
          hs3 hm_eq hv_lt h1_step j₀ rfl hj₀_lt]
        exact hk_eq.symm
