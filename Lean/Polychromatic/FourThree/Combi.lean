...
lemma case_one_res_3g_sub_2 (g : ℕ) (hm : m ≥ 289)
    ...
    HasPolychromColouring (Fin 3)
      ({0, 1, (g : ZMod m), (g : ZMod m) + 1} :
        Finset (ZMod m)) := by
  obtain ⟨u, hu⟩ := ZMod.isUnit_prime_of_not_dvd Nat.prime_three (show ¬3 ∣ m by omega)
  rw [← hasPolychromColouring_mul_unit m u]
  have h3g_mod : (3 : ZMod m) * (g : ZMod m) = 2 := by
    have : ((3 * g : ℕ) : ZMod m) = (m + 2 : ℕ) := by congr 1; omega
    simpa using this
  ...
  simpa [hu, Nat.cast_ofNat, image_insert, mul_zero, mul_one, h3g_mod, image_singleton,
    h3g1_mod, insert_comm] using table1_0235 m (by omega)
...
lemma case_one_res_3g_sub_1 (g : ℕ) (hm : m ≥ 289)
  ...
  simpa [hu, Nat.cast_ofNat, image_insert, mul_zero, mul_one, h3g_mod,
    image_singleton, h3g1_mod, insert_comm] using table1_0134 m (by omega)
...
lemma case_one_div_3g (g : ℕ) (hm_eq : m = 3 * g)
  ...
  · have hcv : c v = (r % 3 + q) % 3 := hv_eq ▸ color_at q r hr_lt
    ...
    have hcvg1 : c (v + g + 1) = ((r + 1) % 3 + (q + 1)) % 3 := by
      rw [show v + g + 1 = g * (q + 1) + (r + 1) from by rw [mul_add, mul_one]; omega]
      exact color_at (q + 1) (r + 1) (by omega)
  ...
  · push_neg at hr_lt_gm1
    ...
    have hcv : c v = (2 + q) % 3 := by
      rw [show v = g * q + r from hv_eq, color_at q r hr_lt, hr_eq, ht]; omega
    have hcv1 : c (v + 1) = (q + 1) % 3 := by
      rw [show v + 1 = g * (q + 1) + 0 from by rw [mul_add, mul_one]; omega,
          color_at (q + 1) 0 hg]; simp
    have hcvg : c (v + g) = (2 + (q + 1)) % 3 := by
      rw [show v + g = g * (q + 1) + (g - 1) from by rw [mul_add, mul_one]; omega,
          color_at (q + 1) (g - 1) (by omega), ht]; omega
...
lemma case_one_div_3g3 (g : ℕ) (hm_eq : m = 3 * g + 3) (hg3 : 3 ∣ g) (hg : 0 < g) :
  ...
  · have hcv : c v = (3 - q % 3) % 3 := by
    ...
    have hcv1 : c (v + 1) = (3 - (q + 1) % 3) % 3 := by
      rw [show v + 1 = h * (q + 1) + 0 from by rw [mul_add, mul_one]; omega,
          color_at (q + 1) 0 (by omega)]
      omega
    have hcvg : c (v + g) = (2 + (3 - (q + 1) % 3)) % 3 := by
      rw [show v + g = h * (q + 1) + (g - 1) from by rw [mul_add, mul_one]; omega,
          color_at (q + 1) (g - 1) (by omega), ht,
          show 3 * t - 1 = 3 * (t - 1) + 2 from by omega]; simp
    exact endgame_witness (Nat.mod_lt _ (by omega)) 0 g 1
      (by simp) (by simp) (by simp)
      hcv (by rw [hcvg]; grind) (by rw [hcv1]; grind)
  · have hcv : c v = (r % 3 + (3 - q % 3)) % 3 := rfl
    have hcv1 : c (v + 1) = ((r + 1) % 3 + (3 - q % 3)) % 3 := by
      rw [show v + 1 = h * q + (r + 1) from by omega, color_at q (r + 1) (by omega)]
    have hcvg1 : c (v + g + 1) = (r % 3 + (3 - (q + 1) % 3)) % 3 := by
      rw [show v + g + 1 = h * (q + 1) + r from by rw [mul_add, mul_one]; omega,
