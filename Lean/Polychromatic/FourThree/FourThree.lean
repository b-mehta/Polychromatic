...
/-!
# Computational Verification for the Four-Three Problem

This file contains the computational infrastructure for verifying the four-three problem
by checking periodic colourings. It provides:

1. Quick acceptance lemmas for cases solvable by residues modulo 3
2. A metaprogramming framework for generating and verifying proof terms
3. Efficient bit vector operations for checking colouring validity

## Main definitions

* `accept_3_*`: Quick acceptance lemmas using residues modulo 3.
* `allA_of_3_*`: Closure lemmas for the `allA` predicate using mod-3 reasoning.
* `mkTable`: Metaprogramming to build a table of verified colourings from external data.
* `prove_allC`: A tactic that proves `allC C` by computational verification.

## Implementation notes

The verification reads colouring data from an external file generated by a C++ program.
Each line encodes a periodic colouring that witnesses the polychromatic property for
a family of quadruples.
-/
